+++
title = "About me"
date = 2026-02-15T19:30:40+07:00
draft = false
_build = { list = false, render = true }
+++

**Education: Military Technical Academy**  
**Major:** Information Security  
**Duration:** 10/2022 – Present  

---

**Experience: Self-study & Research**   
**Role:** Student Researcher  
**Duration:** 03/2023 – Present  

- Practiced Binary Exploitation (basic → advanced) on HackTheBox, pwn.college, picoCTF, pwnable.tw,...  
- Participated in international CTF competitions (CTFtime)  
- Wrote detailed write-ups (EN/VN) published on personal GitHub  

---

**Technical Skills: Binary Exploitation**
- Stack Overflow  
- Heap Exploitation  
- Format String  
- ROP Chain  
- Shellcoding  
- Bypass: NX/DEP, ASLR, PIE, Canary  
- Tools: Pwndbg, Pwntools  
- Architecture: x86, x64  

---

**Awards & Achievements**
- **2024:** Second Prize – MTA CTF 2024  
- **2024:** Consolation Prize – ASEAN Information Security Student Competition  
- **2025:** Third Prize – Attack & Defense MTUCICTF 2025  
- **2025:** Top 8 – Digital Dragon CTF 2025  
- **2025:** Third Prize – Cybersecurity Student Contest Vietnam
---
